...
class Member:
    ...
    def generate_daily_summary(self):
        ...
        try:
            summary_text = run_llm(
                system_prompt,
                user_prompt,
                operation="normal_daily_summary_generation",
            )
    ...
    def send_email(self, activity, current_time, evidence=None):
        recipient_ids = [
            member_id
    ...
    def execute_task(self, activity, current_time):
        ...
        if "break" in activity:
            ...
            if is_email_only_activity(activity):
                # Keep email generation causal: the simulated clock for this
                # employee does not advance until the email is actually
                # generated and delivered.  A bounded semaphore prevents five
                # employee threads from flooding the remote vLLM endpoint.
...
You are loafing around during work. Find a small amount of interesting public information related to your interests, using the available lightweight web tools if useful. Never attempt to solve or bypass a CAPTCHA. Summarize what you found in a few sentences."""
                ...
                if mentioned_member_ids(activity):
                    with AUX_MODEL_SEMAPHORE:
                        self.send_email(
                            activity, current_time, evidence=evidence
                        )
    ...
    def run(self, start_time):
        ...
        # Daily summaries are generated by the main thread only after the
        # activity supervisor has drained.  This prevents summary model calls
        # from competing with active tasks and ensures they describe completed
        # work rather than merely dispatched work.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Daily execution for existing schedules and members."
    )
    parser.add_argument(
        "--date", type=str, required=True, help="Day of the week, e.g., Monday"
    )
    ...
    metadata_path = write_run_metadata(
        log_dir, "normal_daily_execution", event="started", week=week, date=date
    )
    ...
    # Thread exceptions do not normally affect a Python process exit code. Keep
    # an explicit failure list so a broken employee simulation cannot be
    # reported as a successfully completed day.
    thread_failures = []
    ...
    def run_member(member):
        try:
            ...
            STOP_EVENT.set()
            # Do not leave the other employee threads waiting up to the full
            # activity timeout after one task has already made the day fail.
    ...
    for member in members:
        if member.no_more_task:
            continue
        # Normal completion still joins every employee. Daemon mode only
        # prevents a blocked auxiliary call from keeping Python alive after an
        # explicit interrupt and supervisor abort.
    ...
    print("[INFO] Employee schedules dispatched; waiting for activity workers.")
    ...
    if task_failures:
        ...
        raise RuntimeError(
            f"{len(task_failures)} activity task(s) failed or timed out."
        )
    ...
    print(
        f"[INFO] All members have completed their tasks for week {week} - date {date} ."
    )
    write_run_metadata(
        log_dir,
        "normal_daily_execution",
