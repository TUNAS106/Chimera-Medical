...
class BaseBrowser:
    ...
    def init(self) -> None:
        ...
        if self._closed:
            raise RuntimeError("This browser runtime has already been closed.")
        # Launch once and reuse it if browse_url is invoked more than once in
        # the same employee task.
    ...
    def fill_input_id(self, identifier: Union[str, int], text: str) -> str:
        ...
        if not editable:
            raise ValueError(
                "The selected element is not an editable input control."
            )
    ...
    def close(self):
        """Close every Playwright resource; safe to call more than once."""
        ...
        for resource_name in ("page", "context", "browser"):
            ...
            try:
                ...
                logger.debug(
                    "Ignoring error while closing browser resource %s",
                    resource_name,
                    exc_info=True,
                )
            finally:
        ...
        if playwright is not None:
            try:
                ...
                logger.debug(
                    "Ignoring error while stopping Playwright", exc_info=True
                )
            finally:
                self.playwright = None

    def human_verification_reason(self) -> Optional[str]:
        """Return a challenge reason without attempting to solve a CAPTCHA."""

        if self.page is None:
            return None
        try:
            title = self.page.title(timeout=2000)
        except Exception:
            title = ""
        try:
            ...
            body = ""
        challenge_text = f"{title} {body[:5000]}".lower()
        markers = (
            "captcha",
...
class BrowserToolkit(BaseToolkit):
    ...
    def __init__(
        ...
        # Browser startup is deliberately lazy.  Most employee activities do
        # not need interactive browsing, and eager startup previously created
        # one Chromium process tree for every task.
    ...
    def close(self) -> None:
        """Release Chromium and the Playwright Node driver."""
...
"""
    ...
    def browse_url(
        ...
        captcha_reason = self.browser.human_verification_reason()
        if captcha_reason:
            logger.warning(
                "Browser task blocked by human verification: %s", captcha_reason
            )
            return (
                "CHIMERA_BLOCKED_CAPTCHA: the destination requires human "
                "verification. Do not retry or attempt to bypass it; use "
                "search_web, fetch_url, or a different public source."
            )

        configured_round_limit = max(
        ...
        for i in range(round_limit):
            ...
            if captcha_reason:
                ...
                logger.warning(
                    "Browser task blocked by human verification: %s",
                    captcha_reason,
                )
                break
            ...
            if "stop" in action_code:
                ...
                # Repeating the exact same failing action against a dynamic DOM
                # only creates stale-selector loops.  Two failed attempts are
                # enough to force a clean incomplete result/fallback.
                repeated_failures = [
                    item
                    for item in self.history[-2:]
                    if item.get("action") == action_code
                    and not item.get("action_if_success")
                ]
                if len(repeated_failures) >= 2:
                    blocked_reason = "repeated_failed_action"
                    logger.warning(
                        "Stopping repeated browser action after two failures: %s",
                        action_code,
                    )
                    break
        ...
        if blocked_reason == "captcha":
            simulation_result = (
                "CHIMERA_BLOCKED_CAPTCHA: human verification prevented access. "
                "The challenge was not retried or bypassed."
            )
