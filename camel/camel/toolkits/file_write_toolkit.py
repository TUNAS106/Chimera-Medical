...
class FileWriteToolkit(BaseToolkit):
    ...
    def _resolve_filepath(self, file_path: str) -> Path:
        ...
        try:
            resolved_path.relative_to(self.output_dir)
        except ValueError as exc:
            raise ValueError(
                f"Output path must remain inside {self.output_dir}"
            ) from exc
    ...
    def write_file(
        ...
        r"""Write text to a file inside the configured task workspace.

        This concise alias matches the tool vocabulary used reliably by the
        Qwen Hermes parser while retaining ``write_to_file`` compatibility.

        Args:
            path (str): Target path inside the task workspace.
            content (str): Text content to save.
            encoding (Optional[str]): Text encoding; defaults to UTF-8.

        Returns:
            str: Success message or a bounded error description.
        """
