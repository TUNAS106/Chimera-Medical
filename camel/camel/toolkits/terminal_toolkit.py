...
class TerminalToolkit(BaseToolkit):
    ...
    def _is_path_within_working_dir(self, path: str) -> bool:
        ...
        working_dir = os.path.realpath(os.path.abspath(self.working_dir))
        try:
    ...
    def file_find_in_content(
        ...
        if not self._is_path_within_working_dir(file):
            return (
                "Operation restriction: files must be inside the task output "
                f"directory {self.working_dir}."
            )
        if not isinstance(regex, str) or not regex:
            return "Regex must be a non-empty string."
        if len(regex) > 1024:
            return "Regex rejected: maximum length is 1024 characters."
    ...
    def file_read(self, path: str) -> str:
        r"""Read one bounded text file from the task working directory.

        Args:
            path (str): Absolute path inside the task working directory, or a
                path relative to that directory.

        Returns:
            str: File content, truncated at the configured character limit.
        """

        resolved_path = (
            path
            if os.path.isabs(path)
            else os.path.join(self.working_dir, path)
        )
        resolved_path = os.path.realpath(os.path.abspath(resolved_path))
        if not self._is_path_within_working_dir(resolved_path):
            return (
                "Operation restriction: files must be inside the task output "
                f"directory {self.working_dir}."
            )
        if not os.path.isfile(resolved_path):
            return f"File not found or not a regular file: {resolved_path}"

        max_chars = max(
            1000,
            int(os.getenv("CHIMERA_TERMINAL_FILE_READ_MAX_CHARS", "12000")),
        )
        try:
            with open(
                resolved_path, "r", encoding="utf-8", errors="replace"
            ) as input_file:
                content = input_file.read(max_chars + 1)
    ...
    def file_find_by_name(self, path: str, glob: str) -> str:
        ...
        if not self._is_path_within_working_dir(path):
            return (
                "Operation restriction: paths must be inside the task output "
                f"directory {self.working_dir}."
            )
        if not isinstance(glob, str) or not glob or len(glob) > 256:
            return "Glob must be a non-empty string of at most 256 characters."
