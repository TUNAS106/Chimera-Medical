...
class Worker(BaseNode, ABC):
    ...
    async def _listen_to_channel(self):
        ...
        while True:
            ...
            # Process the task. An exception escaping this listener used to
            # terminate the child asyncio task without returning the packet;
            # the parent workforce would then wait forever for RETURNED.
            try:
                task_state = await self._process_task(task, task_dependencies)
            except Exception:
                logger.exception(
                    "%s crashed while processing task %s; returning FAILED.",
                    self,
                    task.id,
                )
