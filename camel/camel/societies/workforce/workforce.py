...
class Workforce(BaseNode):
    ...
    def process_task(self, task: Task) -> Task:
        ...
        # Callers may provide a deterministic decomposition when the number
        # and identity of work items are part of the output contract. Fall
        # back to model-driven decomposition for ordinary Workforce tasks.
        if task.subtasks:
            ...
            # The agent tends to be overconfident on the whole task, so we
    ...
    def _find_assignee(
        ...
        """
        valid_assignee_ids = {child.node_id for child in self._children}
        # Deterministic workflows can bind a predeclared task to a worker.
        # This avoids spending a model call on an assignment that the caller
        # already knows and prevents a valid-but-wrong coordinator choice.
        if task.additional_info:
            try:
                task_info = json.loads(task.additional_info)
            except (json.JSONDecodeError, TypeError):
                task_info = None
            if isinstance(task_info, dict):
                preferred_id = task_info.get("workforce_assignee_id")
                if preferred_id is not None:
                    preferred_id = str(preferred_id)
                    if preferred_id not in valid_assignee_ids:
                        raise ValueError(
                            f"Task {task.id} requests unknown assignee "
                            f"{preferred_id!r}; valid IDs are "
                            f"{sorted(valid_assignee_ids)}."
                        )
                    return preferred_id
        ...
        raw_assignee_id = task_assign_result.assignee_id
        # Worker IDs are displayed to the model as ``<123...>``. Some models
        # preserve those angle brackets in their structured response, while
        # TaskChannel compares IDs literally. Posting such an ID leaves every
        # worker and the workforce waiting on the channel forever. Resolve any
        # valid child ID contained in the response instead of trusting its
        # surrounding formatting.
        response_matches = [
            node_id
            for node_id in valid_assignee_ids
            if node_id in raw_assignee_id
        ]
        if len(response_matches) == 1:
            assignee_id = response_matches[0]
        else:
            assignee_id = raw_assignee_id.strip().strip("<>").strip()

        if assignee_id not in valid_assignee_ids:
            # Decomposition often names the intended worker ID explicitly in
            # the subtask. This is a deterministic fallback and avoids another
            # model call when the coordinator returned malformed output.
            task_matches = [
                node_id
                for node_id in valid_assignee_ids
                if node_id in task.content
            ]
            if len(task_matches) == 1:
                assignee_id = task_matches[0]
                logger.warning(
                    "Coordinator returned invalid assignee ID %r; using "
                    "worker %s referenced by task %s.",
                    raw_assignee_id,
                    assignee_id,
                    task.id,
                )
                ...
                raise ValueError(
                    "Coordinator returned an unknown assignee ID "
                    f"{raw_assignee_id!r}; valid IDs are "
                    f"{sorted(valid_assignee_ids)}."
                )
    ...
    async def _post_task(self, task: Task, assignee_id: str) -> None:
        ...
        if assignee_id not in valid_assignee_ids:
            raise ValueError(
                f"Refusing to post task {task.id} to unknown assignee "
                f"{assignee_id!r}; valid IDs are "
                f"{sorted(valid_assignee_ids)}."
            )
