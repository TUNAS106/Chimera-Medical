...
class ChatAgent(BaseAgent):
    ...
    Args:
        ...
        max_tool_iterations (int): Maximum internal tool/model rounds in one
            step before forcing a final response without tools. (default: 5)
    """
        ...
        while True:
            ...
            if tool_call_requests := response.tool_call_requests:
                ...
                if tool_iterations >= self.max_tool_iterations:
                    logger.warning(
                        "Reached max_tool_iterations=%s; requesting a final "
    ...
    def _execute_tool(
        ...
        max_argument_chars = max(
            512, int(os.getenv("CHIMERA_MAX_TOOL_ARGUMENT_CHARS", "4096"))
        )
        serialized_args = json.dumps(args, ensure_ascii=False, default=str)
        if len(serialized_args) > max_argument_chars:
            result = {
                "error": (
                    f"Tool arguments are too large ({len(serialized_args)} "
                    f"characters; limit {max_argument_chars}). Use a short, "
                    "specific query or regex."
                )
            }
            logging.warning(
                "Rejected oversized arguments for tool '%s': %s characters",
                func_name,
                len(serialized_args),
            )
            return self._record_tool_calling(
                func_name, {}, result, tool_call_id
            )
    ...
    async def _aexecute_tool(
        ...
        if len(serialized_args) > max_argument_chars:
            ...
            logging.warning(
                "Rejected oversized arguments for async tool '%s': "
                "%s characters",
                func_name,
                len(serialized_args),
            )
