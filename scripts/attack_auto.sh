...
if (( EUID != 0 )); then
    ...
    echo "Run: CHIMERA_ATTACKER_ID=<ID> sudo -E bash scripts/attack_auto.sh" >&2
    exit 1
fi
...
for required_command in docker tmux nsenter tcpdump stat mktemp; do
...
if ! docker exec "$CONTAINER_NAME" bash -lc \
    ...
    echo "Model preflight failed. Refresh the Kaggle session/proxy URL in .env; attack/capture was not started." >&2
    exit 1
fi
...
MAX_CONCURRENT_TASKS="${CHIMERA_MAX_CONCURRENT_TASKS:-2}"
if ! [[ "$MAX_CONCURRENT_TASKS" =~ ^[1-9][0-9]*$ ]]; then
    echo "CHIMERA_MAX_CONCURRENT_TASKS must be a positive integer." >&2
    exit 1
fi
echo "Activity supervisor: max_concurrent=$MAX_CONCURRENT_TASKS."
...
CAPTURE_STATUS_DIR="$(mktemp -d /tmp/chimera-attack-capture-status.XXXXXX)"
SCAP_EXIT_FILE=""
PCAP_EXIT_FILE=""
...
stop_container_simulation() {
    ...
    docker exec "$CONTAINER_NAME" bash -lc '
        ...
        if [[ "$command_line" != *"/data/Chimera/src/daily_execution_auto_attack.py"* ]]; then
            rm -f -- "$pid_file"
            exit 0
        fi
...
attacker_id="${CHIMERA_ATTACKER_ID:-}"
if [[ -z "$attacker_id" ]]; then
    echo "Set CHIMERA_ATTACKER_ID to one of the generated two-person profile IDs." >&2
    exit 1
fi
...
read -r -a attacks <<< "${CHIMERA_ATTACK_IDS:-multi_attack_2}"
...
for attack_id in "${attacks[@]}"; do
    ...
    # Select the attack date and inject the attack into the generated schedule.
    docker exec "$CONTAINER_NAME" bash -lc \
        'source /data/Chimera/.venv/bin/activate && exec python /data/Chimera/src/attack_schedule.py --attacker "$1" --attid "$2"' \
        bash "$attacker_id" "$attack_id"
    ...
    # run the simulation
    CONTAINER_SIM_PID_FILE="/tmp/chimera-attack-$$-${attack_id}.pid"
    set +e
    docker exec "$CONTAINER_NAME" bash -lc \
        'pid_file="$1"; attacker="$2"; attack_id="$3"; printf "%s\n" "$$" > "$pid_file"; source /data/Chimera/.venv/bin/activate && exec python /data/Chimera/src/daily_execution_auto_attack.py --attacker "$attacker" --attid "$attack_id"' \
        bash "$CONTAINER_SIM_PID_FILE" "$attacker_id" "$attack_id"
    SIMULATION_RC=$?
    set -e
    ...
    if ! env SYSDIG_PLUGIN_DIR="$SYSDIG_PLUGIN_DIR" \
        "$SYSDIG_BIN" -r "$SCAP_FILE" -n 1 >/dev/null 2>&1; then
        echo "Invalid SCAP output: $SCAP_FILE" >&2
        exit 1
    fi
    if ! tcpdump -nn -r "$PCAP_FILE" -c 1 >/dev/null 2>&1; then
    ...
    if (( SIMULATION_RC != 0 )); then
        echo "Attack simulation failed with exit code $SIMULATION_RC." >&2
        exit "$SIMULATION_RC"
    fi
    rm -f -- "$SCAP_EXIT_FILE" "$PCAP_EXIT_FILE"
    SCAP_EXIT_FILE=""
