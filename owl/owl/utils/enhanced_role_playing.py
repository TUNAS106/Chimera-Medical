...
def _paths_refer_to_same_file(first_path: str, second_path: str) -> bool:
    """Match a toolkit-relative write with its absolute verification read."""

    first = os.path.normpath(first_path)
    second = os.path.normpath(second_path)
    if first == second:
        return True
    if os.path.isabs(first) and not os.path.isabs(second):
        return first.endswith(os.sep + second)
    if os.path.isabs(second) and not os.path.isabs(first):
        return second.endswith(os.sep + first)
    return False


def _artifact_tool_call_succeeded(tool_name: str, result: object) -> bool:
    """Classify file-tool results without inspecting successful file content.

    ``file_read`` returns the file contents directly.  Looking for generic
    phrases such as ``"failed to"`` anywhere in that value therefore produces
    false failures when those words are part of a report or source file.  The
    file toolkits use stable, leading status messages for their own failures,
    so only those envelopes should affect lifecycle validation.
    """

    normalized_name = str(tool_name).lower()
    result_text = str(result).lstrip().lower()
    if normalized_name in {"write_file", "write_to_file"}:
        return result_text.startswith("content successfully written to file:")
    if normalized_name == "file_read":
        failure_prefixes = (
            "file not found or not a regular file:",
            "error reading file:",
            "operation restriction:",
        )
        return not result_text.startswith(failure_prefixes)
    return False
...
def run_chimera_society(
    ...
    for _round in range(round_limit):
        ...
        # Do not trust textual completion markers alone. A task may finish only
        # after a successful file write followed by a read of the same path.
        for tool_record in tool_call_records:
            tool_name = str(tool_record.get("tool_name", "")).lower()
            args = tool_record.get("args", {})
            if not isinstance(args, dict):
                args = {}
            path = args.get("path") or args.get("filename")
            normalized_path = (
                os.path.normpath(path) if isinstance(path, str) else None
            )
            tool_succeeded = _artifact_tool_call_succeeded(
                tool_name, tool_record.get("result", "")
            )
            if (
                ...
                and normalized_path
                and any(
                    _paths_refer_to_same_file(written_path, normalized_path)
                    for written_path in written_artifact_paths
                )
                and tool_succeeded
            ):
                artifact_tool_verified = True
        ...
        assistant_content = _data["assistant"]
        # Track the current (therefore ultimately the final) assistant turn.
        # A marker rejected by the reviewer in an earlier round must not make
        # a later round-limit termination look successful.
        assistant_completion_signal = (
            "CHIMERA_TASK_COMPLETE" in assistant_content
        )
        logger.info(
    ...
    if (
        not completion_signal
        and assistant_completion_signal
        and artifact_tool_verified
    ):
        # The assistant may finish on the final available round, leaving no
        # subsequent reviewer turn in which the user agent can emit TASK_DONE.
        completion_signal = True
        termination_reason = "assistant_completion_marker"
